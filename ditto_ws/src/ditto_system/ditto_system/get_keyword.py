...
RESOURCE_PATH = os.path.join(get_package_share_directory("ditto_system"), "resource")
ENV_PATH = os.path.join(RESOURCE_PATH, ".env")
load_dotenv(dotenv_path=ENV_PATH)
openai_api_key = os.getenv("OPENAI_API_KEY")

############ Voice Status Publishing ############
# 웹 백엔드(backend/routes/voice.py)가 폴링해서 UI에 "웨이크워드 대기중/녹음중/처리중"과
# 인식된 문장을 보여줄 수 있도록, robot_replay.py의 _publish_runtime_status()와 동일한
# 방식(임시 파일 + os.replace로 원자적 교체)으로 상태를 파일에 남긴다.
RUNTIME_DIR = os.path.expanduser("~/Desktop/Dittobot/ditto_ws/.runtime")
_VOICE_STATUS_PATH = os.path.join(RUNTIME_DIR, "voice_status.json")
_VOICE_STATUS_TMP_PATH = _VOICE_STATUS_PATH + ".tmp"
os.makedirs(RUNTIME_DIR, exist_ok=True)


def publish_voice_status(state, **extra):
    payload = {"state": state, "updated_at": time.time(), **extra}
    try:
        with open(_VOICE_STATUS_TMP_PATH, "w") as f:
            json.dump(payload, f, ensure_ascii=False)
        os.replace(_VOICE_STATUS_TMP_PATH, _VOICE_STATUS_PATH)
    except OSError as e:
        print(f"[음성 상태 기록 오류] {e}")
...
class GetKeyword(Node):
    def __init__(self):
        ...
        prompt_content = """
            ...
            <도구 리스트>
            - hammer(망치), screwdriver(드라이버), wrench(스패너), brush(솔), pos1, pos2, pos3
            ...
            - 도구 리스트의 한글 표기(망치/드라이버/스패너/솔)가 사용자 문장에 나오면
              반드시 대응하는 영어 이름(hammer/screwdriver/wrench/brush)으로 출력하세요.

            <특수 규칙>
            - 명확한 도구 명칭이 없지만 문맥상 유추 가능한 경우(예: "못 박는 것" → hammer,
              "먼지 터는 것"/"청소용 붓" → brush)는 리스트 내 항목으로 최대한 추론해 반환하세요.
    ...
    def get_keyword(self, request, response):  # 요청과 응답 객체를 받아야 함    # d2 이 함수 일부 수정함
        ...
        publish_voice_status("listening_for_wakeword")
        while not self.wakeup_word.is_wakeup():
            pass

        # STT --> Keword Extract --> Embedding
        publish_voice_status("recording")
        output_message = self.stt.speech2text()

        publish_voice_status("processing", transcript=output_message)
        keyword = self.extract_keyword(output_message)

        self.get_logger().warn(f"Detected tools: {keyword}")

        publish_voice_status("done", transcript=output_message, tools=keyword)
